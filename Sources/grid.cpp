...
void main() {
    ...
    // Constructor
    Grid::Grid(float width,  float depth,  float height,  int numDivisions) {
        ...
        if (Grid::gridInstances == 0) {
            ...
            Grid::gridShaderProgram = new ShaderProgram();

            Grid::gridShaderProgram->initFromStrings(Grid::vertexShaderSource, Grid::fragmentShaderSource);

            // ----- Grid shader attributes and uniforms -----

            // Add the shader attributes
            Grid::gridShaderProgram->addAttribute("vertexLocation");

            // Add the shader uniforms
            Grid::gridShaderProgram->addUniform("mvpMatrix");
        }
        ...
        float xStep = width / (float)numDivisions;
        float zStep = depth / (float)numDivisions;
        ...
        float gridArraySizeBytes = numVerts * VERTEX_COMPONENTS * sizeof(float);
        glBufferData(GL_ARRAY_BUFFER, gridArraySizeBytes, gridArray, GL_STATIC_DRAW);
    ...
    Grid::~Grid() {
        ...
        // Decrement out count of grid instances
        gridInstances--;
