...
namespace ComS342 {
    ...
    void Camera::handleKeypress(GLint key, GLint action) {
        ...
        // Note: The GLFW_REPEAT check helps us to not lose keypresses
        // when pressing and releasing multiple keys at once.
        if (action == GLFW_PRESS || action == GLFW_REPEAT) {
            ...
            // If a key is released, toggle the relevant flag
            switch (key) {
                case GLFW_KEY_W:
                    holdingForward = false;
                    break;
